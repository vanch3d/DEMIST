...
//
//
void CMultiRectTracker::Add( CAnimObject* pObject )
{
    m_Objects.Add (pObject);
    m_pObject = pObject;
}

void CMultiRectTracker::Remove( CAnimObject* pObject )
{
	if (!pObject) return;

    for (int index = 0; index < m_Objects.GetSize(); index++)
	{
		CAnimObject *pObj = m_Objects.GetAt(index);
		if (pObj == pObject)
		{
...
void CMultiRectTracker::Draw(CDC* pDC)
{
	...
	//m_pView->OnPrepareDC(&dc);
    for (int index = 0; index < m_Objects.GetSize(); index++) {
        m_rect.CopyRect(m_Objects.GetAt(index)->GetRect());
		pDC->LPtoDP(&m_rect);
...
void CMultiRectTracker::CopyPositions ()
{
    ...
    for (int index = 0; index < m_Objects.GetSize(); index++)
	{
		CRect mrect2 = m_Objects.GetAt(index)->GetRect();
