...
public:
    ...
    void Add( CAnimObject* pObject );
    void Remove( CAnimObject* pObject );
    void RemoveAll();
    bool IsEmpty() { return (m_Objects.GetSize() == 0); }
	int	 GetSize() { return m_Objects.GetSize();};
	CAnimObject *GetAt(int i) { return m_Objects.GetAt(i);};
...
protected:
	CSize m_szGrid;
	CView *m_pView;
    CArray <CAnimObject*, CAnimObject*> m_Objects; // Array of objects pointers
    CAnimObject* m_pObject; // temporary pointer used if the hit is on an object.
