...
#define TABWND_HEIGHT     30    // Height of the gray border between the toolbar 
                                // and the client area
#define TAB_HEIGHT        22    // Height on the normal tab
#define TABSEL_HEIGHT     22    // Height of the selected tab
...
int TTabWnd::drawSelTabBottom(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
  ...
  if (pItem->m_hIcon) {
    ::DrawIconEx(pDC->m_hDC,x+4,y+TAB_ICON_Y-1,pItem->m_hIcon,16,16,0,NULL,DI_NORMAL);
  }
...
// Optional: Set icon for tab and load it from the resource
BOOL TVisualObject::SetIcon(HICON hIcon)
{
  // Cannot specify icon if already specified
  if (m_hIcon)
    return FALSE;
