...
// OptionTree.cpp : implementation file
//
...
CRect COptionTree::GetLargestLabel()
{
	// Set as negative
	m_rcLargestLabel = CRect(0, 0, 0, 0);

	// Enumerate items
	EnumItems(&m_otiRoot, EnumGetLargestLabelRect, NULL);
...
BOOL CALLBACK COptionTree::EnumGetLargestLabelRect(COptionTree* otProp, COptionTreeItem* otiItem, LPARAM lParam)
{
	// Validate items
	if (otiItem == NULL)
	{
...
LRESULT COptionTree::SendNotify(UINT uNotifyCode, COptionTreeItem* otiItem)
{
	...
	if (lpnm != NULL)
	{
		...
		return GetParent()->SendMessage(WM_NOTIFY, (WPARAM)uID, (LPARAM)lpnm);
	}
...
void COptionTree::SetNotify(BOOL bNotify)
{
	// Declare variables
	CRect rcClient;
