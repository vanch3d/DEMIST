...
CColourPopup::CColourPopup(CPoint p, COLORREF crColour, CWnd* pParentWnd,
						   ...
						   COLORREF crDefault /*= CLR_DEFAULT*/)
{
    Initialise();
...
{
    if (crColour == m_crDefault/*CLR_DEFAULT*/ && m_strDefaultText.GetLength())
    {
...
void CColourPopup::ChangeSelection(int nIndex)
{
    ...
    {
        m_crColour = m_crDefault/*CLR_DEFAULT*/;
        m_pParent->SendMessage(CPN_SELCHANGE, (WPARAM) m_crDefault/*CLR_DEFAULT*/, 0);
    }
    else
    {
...
void CColourPopup::DrawCell(CDC* pDC, int nIndex)
{
    ...
    if (m_strDefaultText.GetLength() && nIndex == DEFAULT_BOX_VALUE)
    {
        ...
        // fill background
        if (m_nChosenColourSel == nIndex && m_nCurrentSel != nIndex)
            pDC->FillSolidRect(TextButtonRect, (m_crDefault==CLR_DEFAULT) ? 
								::GetSysColor(COLOR_3DLIGHT) : m_crDefault);
        else
            pDC->FillSolidRect(TextButtonRect, (m_crDefault==CLR_DEFAULT) ? 
								::GetSysColor(COLOR_3DFACE) : m_crDefault);//::GetSysColor(COLOR_3DFACE));
