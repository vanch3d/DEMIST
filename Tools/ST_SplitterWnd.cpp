...
IMPLEMENT_DYNAMIC(ST_SplitterWnd, CSplitterWnd)   // for swap tuning
...
void ST_SplitterWnd::SaveToRegistry()
{
   ...
   RememberSize();
   if (m_nPaneSize[0] == -1) m_nPaneSize[0] = 150;
   if (m_nPaneSize[1] == -1) m_nPaneSize[1] = 150;
...
void ST_SplitterWnd::RefreshSplitBars(void)
{
    CRect rectInside;
...
void ST_SplitterWnd::OnDrawSplitter(CDC* pDC, ESplitType nType, const CRect& rectArg)
{
/*	int x_ActivePane, y_ActivePane; 
	COLORREF hilightcolor = RGB(255,255,0);
	CWnd *pWnd = GetActivePane(x_ActivePane, y_ActivePane);
	
	if( pWnd && (nType == splitBorder))
	{
		int nn = GetRowCount();
		int bb = GetColumnCount();
		int pRow = 0;
		int pCol = 0;
		if(rectArg.top)
		{
...
/*	int x_ActivePane, y_ActivePane; 
	...
	if (m_pSubSplitterWnd[1])
	{
	 ...
	 if ((FOCUS_HILIGHT_SHOW) && ((GetRowCount() > 1) || (GetColumnCount() > 1)))
    {
        int pRow = 0;
        int pCol = 0;
        if (rectArg.top)
        ...
        if ((x_ActivePane == pRow) && (y_ActivePane == pCol))
        {
            if (pDC == NULL)
            {
                RedrawWindow(rectArg, NULL, RDW_INVALIDATE|RDW_NOCHILDREN);
...
void ST_SplitterWnd::OnContextMenu(CWnd* pWnd, CPoint point) 
{
...
LRESULT ST_SplitterWnd::OnDoContextMenu(WPARAM wp, LPARAM lp)
{
...
void ST_SplitterWnd::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void ST_SplitterWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if (!m_bBarLocked)
...
BOOL ST_SplitterWnd::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	// TODO: Add your message handler code here and/or call default
	if (!m_bBarLocked)
